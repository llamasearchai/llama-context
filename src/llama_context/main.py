...
class ContextEngine:
    ...
    def _init_guardrails(self) -> List[BaseGuardrail]:
        ...
        if self.config.guardrail_level in [GuardrailLevel.MEDIUM, GuardrailLevel.HIGH]:
            guardrails.append(
                LLMVerificationGuardrail(
                    provider=self.provider, level=self.config.guardrail_level
                )
            )
...
class AnthropicProvider(BaseProvider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(
            api_key, model, max_tokens, temperature, top_p, timeout, **kwargs
        )
    ...
    async def generate_with_json(
        ...
        try:
            ...
            self.logger.error(
                f"Error generating JSON response from Anthropic: {str(e)}"
            )
            raise
